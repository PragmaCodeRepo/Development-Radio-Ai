...
class SchedulingAdmin(admin.ModelAdmin):
     list_display=('sftp_host','sftp_port','sftp_username','sftp_password','sftp_remote_path','rss_url','limit','schedule_time','recurrence_type', 'voice','intros', 'outros', 'is_pending','created_at') 
admin.site.register(SchedulingTasks,SchedulingAdmin)   
    

class IntrosAdmin(admin.ModelAdmin):
    list_display = ('intros','created_at')
class OutrosAdmin(admin.ModelAdmin):
    list_display = ('outros','created_at')
